...
document.addEventListener('DOMContentLoaded', () => {
  ...
  form.addEventListener('submit', e=>{
    ...
    if(!nameEl.value.trim()){ setErr(nameEl,'Requis'); ok=false; } else setErr(nameEl,'');
    const ev=emailEl.value.trim();
    if(!ev){ setErr(emailEl,'Requis'); ok=false; }
    else if(!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(ev)){ setErr(emailEl,'Invalide'); ok=false; }
    else setErr(emailEl,'');
    if(!msgEl.value.trim()){ setErr(msgEl,'Requis'); ok=false; } else setErr(msgEl,'');
    ...
    const subject='Prise de contact depuis le portfolio';
    const body=`Nom: ${nameEl.value.trim()}\nEmail: ${ev}\n\n${msgEl.value.trim()}`;
    const mailto=`mailto:doae.oudadda@gmail.com?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;

    // Trigger mail client
    window.location.href=mailto;

    status.textContent='Ouverture de votre messagerie... Si rien ne s’ouvre, contactez-moi directement par email.';
